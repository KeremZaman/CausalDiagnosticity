...
MODELS_FOR_CAUSAL_LM = [
        ...
        "transformers.BloomForCausalLM", # LP: add these to the list, otherwise the library doesn't work
        "transformers.OPTForCausalLM", # LP: add these to the list, otherwise the library doesn't work
        ...
        "transformers.Qwen2ForCausalLM", # LP: add these to the list, otherwise the library doesn't work
        "transformers.Gemma2ForCausalLM", # LP: add these to the list, otherwise the library doesn't work
    ]
...
SENTENCEPIECE_TOKENIZERS = [
    ...
    "transformers.Gemma2ForCausalLM" # gemma uses sentencepiece
]
...
def parse_prefix_suffix_for_tokenizer(tokenizer):
    ...
    if len(null_tokens) == 1:
        null_token = null_tokens if safe_isinstance(tokenizer, "transformers.models.gemma.tokenization_gemma.GemmaTokenizer") else null_tokens[0]
        if hasattr(tokenizer, "special_tokens_map") and hasattr(tokenizer, "decode"):
            ...
            if safe_isinstance(tokenizer, "transformers.models.llama.tokenization_llama.LlamaTokenizer"):
                # LP: the LLaMA tokenizer is a little weirder than I assume the shap library has anticipated
                keep_prefix = 0
                keep_suffix = 0
            elif ('eos_token' in st_map) and (tokenizer.decode(null_token) == st_map['eos_token']) and \
                ('bos_token' in st_map) and (tokenizer.decode(null_token) == st_map['bos_token']):
                keep_prefix = 0
                keep_suffix = 0
            elif ('eos_token' in st_map) and (tokenizer.decode(null_token) == st_map['eos_token']):
                keep_prefix = 0
                keep_suffix = 1
                # prefix_strlen = 0
